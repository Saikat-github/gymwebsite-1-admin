...
const PaymentsPage = () => {
  ...
  const getPayments = async (lastDocument=null) => {
    try {
      setLoader(true);
      const { startDate, endDate } = getStartEndDate(selectedFilter.toLowerCase());
      const result = await getDocuments("payments", null, lastDocument, 10,
        [
  ...
  const loadMore = async () => {
    ...
    const result = await getPayments(lastDoc);
    if (result) {
  ...
  return (
    <div className="py-10 px-6">
      <div className="max-w-6xl mx-auto space-y-8">
        <h1 className="text-3xl font-semibold text-center">All Payments</h1>
        ...
        <div className="flex flex-col gap-6 items-center">
          {payments?.length > 0
            ...
            :
            (
              <p className="text-center text-gray-400">No payments found for {selectedFilter}.</p>
            )}
